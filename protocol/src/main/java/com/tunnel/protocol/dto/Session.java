 ...
 * @param pathPatterns     comma-separated Ant patterns this session intercepts, or
 *                         {@code null} / {@code "ALL"} to intercept everything
 * @param ingressName      name of the temporary Kubernetes Ingress created for this
 *                         session, or {@code null} if no ingress was requested
 */
public record Session(
    ...
    public Session withPod(String newPodId) {
        return new Session(sessionId, subdomain, ownerId, newPodId, SessionStatus.ACTIVE, createdAtEpochMs, pathPatterns, ingressName);
    }

    /** Copy with a new status. */
    public Session withStatus(SessionStatus newStatus) {
        return new Session(sessionId, subdomain, ownerId, podId, newStatus, createdAtEpochMs, pathPatterns, ingressName);
    }
}
