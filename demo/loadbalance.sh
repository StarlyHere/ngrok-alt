#!/usr/bin/env bash
# loadbalance.sh — Demonstrate least-connections load balancing across tunnel pods.
...
if [ "$pod_count" -lt 2 ]; then
  fail "Expected 2 tunnel pods registered in Redis, found $pod_count. Deploy with demo/deploy.sh"
fi
