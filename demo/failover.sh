...
# Restart the SSH port-forward loop right before killing the pod so a fresh
# loop is guaranteed to be running when the connection drops.
...
if [ -n "$REASSIGNED" ]; then
  ...
  fail "Session did not reassign within 60 s. Check Coordinator logs: kc -n tunnel logs deploy/spr-local-connect-coordinator"
fi
...
for i in $(seq 1 30); do
  running=$(kc -n "$NS" get pods -l app=spr-local-connect-relay --no-headers 2>/dev/null \
    | grep -c 'Running' || true)
  [ "$running" -ge 2 ] && break
  sleep 2
done
