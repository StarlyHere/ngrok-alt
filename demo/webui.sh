...
#   Browser → Temporary Ingress ({subdomain}.tunnel.local → LocalConnect router)
#           → LocalConnect router reads remoteDebugConf cookie
...
step "Checking ingress routes to LocalConnect router (not injecting session header)"
...
if echo "$response2" | grep -q '"service":"webui-sim"'; then
  ok "PASS — cookie present → LocalConnect router tunneled request to webui-sim on your laptop"
