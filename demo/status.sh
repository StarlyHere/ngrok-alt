...
for svc in redis spr-local-connect-coordinator spr-local-connect-router qa6-gateway; do
...
tunnel_pods=$(kc -n "$NS" get pods -l app=spr-local-connect-relay --no-headers 2>/dev/null \
  | grep -c 'Running' || true)
ok "relay  ×$tunnel_pods running"
