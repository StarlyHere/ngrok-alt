...
step "Assignment counters  (coordinator:8090/actuator/prometheus)"
raw=$(curl_pod "demo-metrics-$$" "http://spr-local-connect-coordinator:8090/actuator/prometheus")
...
if [ -z "$first_pod" ]; then
  ...
  if [ "${ttl2:-0}" -ge $((${ttl1:-0} - 3)) ]; then
    ...
    info "Check: minikube kubectl -- -n tunnel logs deploy/spr-local-connect-coordinator | grep heartbeat"
  fi
fi
