...
sed "s|TUNNEL_URL_PLACEHOLDER|ssh://localhost:30022|g" \
  deploy/k8s/20-coordinator.yaml | kc apply -f - >/dev/null
kc apply -f deploy/k8s/30-router.yaml >/dev/null
kc apply -f deploy/k8s/16-webui-placeholder.yaml >/dev/null

for svc in redis redpanda spr-local-connect-coordinator spr-local-connect-router webui-placeholder; do
...
# ── §4c  2 relays ────────────────────────────────────────────────────────
step "Deploying 2 relays"
sed 's/replicas: 5/replicas: 2/' deploy/k8s/40-relay.yaml | kc apply -f - >/dev/null
...
step "Starting port-forwards (coordinator :30092, spr-local-connect-relay-ssh :30022)"
...
ok "Port-forwards ready  (coordinator :30092, spr-local-connect-relay-ssh :30022, nginx :8888)"
