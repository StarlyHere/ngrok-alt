...
if [ "$TOPIC_FOUND" = "false" ]; then
  ...
  info "Is KAFKA_ENABLED=true on the coordinator pod?"
  info "Check: minikube kubectl -- -n tunnel logs deploy/spr-local-connect-coordinator | grep kafka"
fi
...
while [ $ASSIGN_WAIT -lt 20 ]; do
  if kc -n "$NS" logs deploy/spr-local-connect-router --tail=20 2>/dev/null \
...
divider
kc -n "$NS" logs deploy/spr-local-connect-router --tail=30 2>/dev/null | grep -i "kafka" || true
divider
