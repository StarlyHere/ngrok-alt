...
public class ConnectionRegistry {
    ...
    /**
     * Remove a session only when it is still owned by the closing connection.
     * A reconnect may already have replaced the map entry; an older socket must
     * not unregister that newer connection when it finally closes.
     */
    public void unregister(String sessionId, PodConnection connection) {
        if (sessionId != null && connection != null && bySession.remove(sessionId, connection)) {
